...
const JacketsMarketplace = () => {

    const [jacketsOnMarketPlace, setJacketsOnMarketplace] = useState<any | null>(null)
    ...
    const svgUrl: string = '/jacket.svg';

    const get_all_jackets = async () => {

        const runaway_marketplace_contract = new Contract(runawayMarketPlaceAbi, runawayMarketplaceContractAddress,provider)

        const jackets = await runaway_marketplace_contract.get_all_jackets();
        ...
        jackets.forEach((jacket: any) => {
        
            const modifiedSvg = changeSkinPathFillType(svg, jacket.jacket.color,pathId);
            jacket.svg = modifiedSvg;
        ...
        setJacketsOnMarketplace([...jackets]);
  ...
  return (
    ...
    <div className="mx-auto max-w-2xl px-4 sm:px-6 sm:py-4 lg:max-w-7xl lg:px-8">
            ...
            <h5 className='text-green-400'> Jackets On Marketplace </h5>
            {jacketsOnMarketPlace ? (
                    <>
                <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                    {jacketsOnMarketPlace.map((jacket: any, index: any ) => (
                      ...
                      <p className="text-sm font-medium text-gray-900 py-2"><span className='text-slate-500'>{feltToString(jacket.price)}</span> ETH</p>
...
export default JacketsMarketplace