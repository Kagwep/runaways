...
const Pants = () => {
    ...
    const [pantsOnMarketPlace, setPantsOnMarketplace] = useState<any | null>(null)
    ...
    const svgUrl: string = '/pants.svg';


    const get_all_pants = async () => {

        const runaway_marketplace_contract = new Contract(runawayMarketPlaceAbi, runawayMarketplaceContractAddress,provider)

        const pants = await runaway_marketplace_contract.get_all_pants();
        ...
        pants.forEach((pant: any) => {
        
            const modifiedSvg = changeSkinPathFillType(svg, pant.pant.color,pathId);
            pant.svg = modifiedSvg;
  ...
  return (
    ...
    <div className="mx-auto max-w-2xl px-4 sm:px-6 sm:py-4 lg:max-w-7xl lg:px-8">
            ...
            <h5 className='text-green-400'> Pants On Marketplace </h5>
            {pantsOnMarketPlace ? (
                    <>
                <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
                    {pantsOnMarketPlace.map((pant: any, index: any ) => (
                      ...
                      <p className="text-sm font-medium text-gray-900 py-2"><span className='text-slate-500'>{feltToString(pant.price)}</span> ETH</p>
